...
GATEWAY_ARGS=(--config "$SCRIPT_DIR/freesail-gateway.config.json")
[ -n "${GATEWAY_PORT:-}" ] && GATEWAY_ARGS+=(--http-port "$GATEWAY_HTTP_PORT")
[ -n "${MCP_PORT:-}" ]     && GATEWAY_ARGS+=(--mcp-port "$GATEWAY_MCP_PORT")
